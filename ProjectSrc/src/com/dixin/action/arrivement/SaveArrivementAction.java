 ...
 * 
 */
public class SaveArrivementAction extends GenericSaveAction<Arrivement> {
	public SaveArrivementAction() {
	...
	public boolean process(HttpServletRequest request, JSONObject result) {
		...
		if (arrive.getDoubleCheck() == Bool.TRUE
				|| oldarrive.getDoubleCheck() == Bool.TRUE) {
			callAction("saveDoubleCheckedArrivementAction");
		}
		...
		for (int i = 0; i < detailsIds.length; i++) {
			...
			d.setId(Integer.valueOf(detailsIds[i]));
