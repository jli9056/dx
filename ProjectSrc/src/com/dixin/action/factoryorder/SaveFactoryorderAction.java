 ...
 * 
 */
public class SaveFactoryorderAction extends GenericSaveAction<Factoryorder> {
	public SaveFactoryorderAction() {
	...
	public boolean process(HttpServletRequest request, JSONObject result) {
		...
		if (oldOrder.getFinished() == Bool.TRUE) {
			// do nothing when factory order was finished
			return true;
		}
		
		if (order.getDoubleCheck() == Bool.TRUE
				|| oldOrder.getDoubleCheck() == Bool.TRUE) {
			callAction("saveDoubleCheckedFactoryorderAction");
		}
		order.setFactoryorderdetails(oldOrder.getFactoryorderdetails());
		...
		for (Iterator i = order.getFactoryorderdetails().iterator(); i
				.hasNext();) {
			Factoryorderdetail od = (Factoryorderdetail) i.next();
