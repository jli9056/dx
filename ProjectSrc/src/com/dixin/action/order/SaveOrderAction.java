 ...
 * 
 */
public class SaveOrderAction extends GenericSaveAction<Order> {
	...
	public boolean process(HttpServletRequest request, JSONObject result) {
		...
		if (OrderStatus.FINISHED.equals(oldOrder.getStatus())) {
			// do nothing if order was finished
			return true;
		}
		
		if (order.getDoubleCheck() == 1 || oldOrder.getDoubleCheck() == 1) {
			callAction("saveDoubleCheckedOrderAction");
			return true;
		}
